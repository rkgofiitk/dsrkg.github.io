...
class App(tk.Tk):
    def __init__(self):
        super().__init__()
        self.title("Stack Animation") # Set the title
        self.geometry("850x800") # Display window size

        self.delay = 10 # Default value for delay
        self.widgets = Stack_and_Queue(self) # Instance of widget class


        self.mainloop()  # Repeat main loop
...
class Stack_and_Queue:
    def __init__(self, parent):
        ...
        self.stack_obj = Stack()  # Set a stack instance
        self.queue_obj = QueueArray()  # Set a queue instance
    ...
    def create_widgets(self):
        ...
        self.subtitle_lb = tk.Label(self.head_frame, 
                                    text='Stack & Queue Animation', 
                                    bg=self.bg_color,
                                    fg='white', font=('Regular', 15)
                                    )

        self.subtitle_lb.place(relx=0.35, rely=0.2)


        # Menu specification available in BottomMenu class
        self.bottom_menu = BottomMenu(self.parent, self)

        # Define canvas display frame and the canvas
        ...
        self.vertical_spacing = self.item_size * 0.75  # vertical spacing 
    ...
    def execute_status_message(self, message, x=180, y=500, color="blue"):
        ...
        # Display/update a persistent status message at position (x,y).
        if not hasattr(self, "status_text_id") or not self.canvas.type(self.status_text_id):
            ...
            # Update existing text to be displayed
    ...
    # Function sets the menu bottons and entry feilds
    def setup_animation_ui(self, var):
        ...
        # Configure create empty stack button
        elif var == "New stack":
        ...
        # Configure inputs according to enqueue operation 
        elif var == "Enqueue":
        ...
        # Configure create empty queue button
        elif var == "New queue":
    ...
    # Create stack or queue display on canvas
    def execute_print(self, var):
        if self.home_btn.winfo_ismapped():
            self.home_btn.place_forget()

        self.canvas.delete("all") # Temporarily clear the canvas

        if var == "Stack":
            self.stack_obj.makenull() # Set an empty stack
            self.draw_stack()
            return

        elif var == "Queue": 
            self.queue_obj.makenull() # Set an empty queue
            self.draw_queue()
            return

    def draw_queue_boders(self, x, y, sep = 1):

        self.canvas.create_line(x, y - 10, x + 610, y-10, 
                                fill="black", width=3)
        self.canvas.create_line(x, y+40, x + 610, y+40, 
                                fill="black", width=3)


    # Draw a separator line between two consecutive elements
    def draw_queue_elem_separator(self, x, y):
        spacing = 70

        x = x + spacing
        self.canvas.create_line(x-10, y-10, x-10, y+40,
                                fill="black", width=3)

    # Draw qfront pointer
    def set_qfront(self, x, y):

        # Front arrow
        self.canvas.create_line(x + 40, y + 5, x + 40, y - 25,
                                fill="black", width=3, arrow="first")
        self.canvas.create_text(x + 40, y - 35,
                                text="front", font=("Arial", 15), fill="black")
    # Draw qrear pointer
    def set_qrear(self, x, y):

        # Rear arrow
        self.canvas.create_line(x - 15, y - 30, x - 15, y,
                                fill="black", width=3, arrow="first")
        self.canvas.create_text(x - 15, y + 5,
                                text="rear", font=("Arial", 15), fill="black")

    # Draw queue
    def draw_queue(self):
        # Make canvas frame visible
        ...
        # Create list of queue elements
        qlist = self.queue_obj.to_list()

        # Set initial coordiates and spacing
        item_width = 50
        spacing = 20
        x_start = 100
        y1, y2 = 310, 340

        # Draw queue boders as two parallel lines on canvas
        self.draw_queue_boders(x_start, y1)

        if self.queue_obj.is_empty():

            # Show empty queue message only when truly empty
            msg = "Queue is empty"
            x_m, y_m = 250, 120
        ...
        # Queue is nonempty, draw it with front and rear pointers
        for i, val in enumerate(qlist):
            if i > 0:
                self.draw_queue_elem_separator(x1, y1)
            x1 = x_start + i * (self.item_size + spacing)
            x2 = x1 + item_width
            self.canvas.create_rectangle(x1, y1, x2, y2,
                                     fill="blue", tags=f"block_{i}")
            self.canvas.create_text((x1+x2)//2, (y1+y2)//2,
                                    text=str(val), font=('Bold', 15),
                                    fill="white", tags=f"text_{i}")

        # Set front pointer
        x_front = x_start - 15
        y_front = y1 - 15

        self.set_qfront(x_front , y_front)

        # Set rear pointer
        x_rear = x2 - 15
        y_rear = y2 + 40
        self.set_qrear(x_rear, y_rear)

        # Print status message for non-empty queue
        msg = f"Queue created with {len(qlist)} elements"
        x_m, y_m = 250, 120
        self.execute_status_message(msg, x_m, y_m, color="blue")
    ...
    def animate_enqueue(self):
        ...
        # Enqueue requires an input
        user_input = self.bottom_menu.data_entry1.get()

        # Check if the input is numeric or not
        if user_input.isnumeric():
            value = int(user_input)
            success = self.queue_obj.enqueue(value) # Enqueue succeeds or not?

            if success: # Animate enqueue if successful 
                spacing = 20
                ...
                # Move new element to left to settle at queue rear  
                def move_left():
                    # Fix the coordinates
                    current_coords = self.canvas.coords(rect_id)
                    # Move is steps of 5 until reaching rear end
                ...
                # Remove the old input from entry field
                self.bottom_menu.data_entry1.delete(0, tk.END)

            else:
                # Enqueue unsuccessful due to full queue condition 
    ...
    def animate_dequeue(self):
        ...
        # Try to find the front block/text
        block_items = self.canvas.find_withtag("block_0")
        text_items = self.canvas.find_withtag("text_0")

        if not (block_items and text_items):
            # Safety check: canvas tags missing
            self.execute_status_message("Queue is empty: Dequeue failed")
            return
        ...
        self.pulsate(block_id, x_center, y_center, pulses=6, callback=move_left)
    ...
    def animate_pop(self):
        ...
        if self.stack_obj.is_empty():
            self.execute_status_message("Stack is empty: pop failed", 
                                        180, 500, color="red")
            return
        ...
        if block_id and text_id:
            ...
            self.pulsate(block_id, x_center, y_center, pulses=6, 
                         callback=move_up)
    ...
    def draw_stack(self):
        ...
        stack_list = self.stack_obj.get_stack()

        for i, val in enumerate(stack_list):
            y1 = base_y - i * self.vertical_spacing
            y2 = y1 + self.block_height

            self.canvas.create_rectangle(x1, y1, x2, y2,
                                         fill="blue", tags=f"block_{i}")
            self.canvas.create_text((x1+x2)//2, (y1+y2)//2, text=str(val),
                                    fill="white", font=('Regular',15), 
                                    tags=f"text_{i}")

            # Separator: draw at midpoint between current and next block
            if i < len(stack_list) - 1:
                sep_y = y2 - (self.vertical_spacing + self.block_height) // 2
                self.draw_stack_elem_separator((x1+x2)//2, sep_y)

        # Draw top arrow
        if self.stack_obj.is_empty():
            self.canvas.create_line((x1+x2)//2, base_y+35, (x1+x2)//2, base_y,
                                fill="black", width=3, arrow="first")
            self.canvas.create_text((x1+x2)//2, base_y-10, text="Top",
                                font=("Arial", 12), fill="black")
        else:
            top_y = base_y - (len(stack_list)-1) * self.vertical_spacing
            self.canvas.create_line((x1+x2)//2, top_y, (x1+x2)//2, top_y-40,
                                fill="black", width=3, arrow="first")
            self.canvas.create_text((x1+x2)//2, top_y-50, text="Top",
                                font=("Arial", 12), fill="black")

        self.draw_stack_boder(x1, x2)

        msg = "Stack drawn"
        self.execute_status_message(msg, 250, 120, color="blue")
    ...
    def draw_stack_elem_separator(self, x, y):
        self.canvas.create_line(x-60, y, x+60, y, fill="black", width=3)
    ...
    def animate_push(self):
        ...
        if self.stack_obj.is_full():
            self.execute_status_message("Stack is full: Push failed", 
                                        350, 500, color="red")
            return
        ...
        if not success:
            self.execute_status_message("Push failed unexpectedly", 350, 
                                        500, color="red")
            return

        # Starting position just above stack boder
        x_center = 350
        y_start = 30
