...
class BinaryTree:
    ...
    def generate_list(self, num_nodes):
        #num_nodes = int(input("Enter number of nodes: "))
        return random.sample(range(1, 100), num_nodes)
  
    # Creates a binary tree with random value by levelwise 
    # insertion method
    def create_tree(self, num_nodes):
        l = self.generate_list(num_nodes)
        print(l)
        for num in l:
    ...
    def _inorder_helper(self, node):
        ...
        result = []
        result.extend(self._inorder_helper(node.left))
        result.append(node)   # <-- append the Node itself
        result.extend(self._inorder_helper(node.right))
        return result
    ...
    def _preorder_helper(self, node):
        ...
        result = []
        result.append(node) # changed from node.data
