...
class Hash:
    ...
    def collision_count(self):
        """Return the number of buckets with collisions (more than one node)."""
        count = 0
        for bucket in self.table:
            if bucket.head and bucket.head.next:  # at least 2 nodes
                count += 1
        return count
...
#def generate_random_array(size=20, max_value=400):
