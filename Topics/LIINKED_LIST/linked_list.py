...
class LLAnimation:
    ...
    # Defines all widgets for buttons, entries and labels 
    def create_widgets(self):
        self.bg_color = "#383839" 

        self.delay = 50 # Animation delay
        self.rect_positions = [] # For storing rectangle positions
        ...
        # Fix scrollbar size and placement
        ...
        # One operation button configured dynamically for linked list operations 
    ...
    def clear_main_content_area(self):
        ...
        if self.setup_btn.winfo_ismapped(): 
    ...
    def setup_animation_ui(self, var):
        ...
        # Three types of operation: requiring two inputs, one input and no input

        if var in ["Append", "Prepend", "Delete", "Find"]: 
            self.data_entry1.grid_forget()
            self.data_entry2.grid_forget()
            self.txt_label1.grid_forget()
            self.txt_label2.grid_forget()

            # Requires single input from user
            ...
            self.setup_btn.configure(text=f"{var}", 
                    command=lambda: self.animate_command(var))

        if var in ["Reverse", "Print List"]:
            # Requires no input from user
            self.setup_btn.grid(row=0,column=0, padx=100, pady=10)
            self.setup_btn.configure(text=f"{var}", 
                    command=lambda: self.execute_command(var))
            self.data_entry1.grid_forget()
            self.data_entry2.grid_forget()
            self.txt_label1.grid_forget()
            self.txt_label2.grid_forget()
        ...
        if var == "Insert": 
            ...
            self.setup_btn.configure(text="Insert At", 
                    command=lambda: self.link_modify_insert())
        ...
        # Define new canvas for the operation
    ...
    def draw_linked_list(self):
        ...
        # Form a list of elements in the linked list
        qlist = self.list_obj.to_list()
        size = self.list_obj.length()
        print("Size of list =", size) # Debug print to see list is ok 
        ...
        # Retrieve one element at a time and draw it on canvas
        for i, val in enumerate(qlist):
            ...
            # Place the text to overlap the rectangle on the center
        ...
        # Now create a header pointer for the list and
        # create link to the first element from the header
    ...
    # Creates movement to right and up
    # Only needed for animating the reverse operation 
...
class SidebarMenu:
    ...
    def create_menu(self):
        ...
        subList3.add_command(label="Linked list", 
                command=lambda: self.widgets.open_pdf("Linked list"))
