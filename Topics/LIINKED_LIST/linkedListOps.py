...
class LinkedList:
    ...
    def deleteAt(self, pos):
        ...
        if pos == 1:  # Special case: Deleting the head
            x = str(self.head.data)
        ...
        while curr is not None:
            if cnt == pos - 1:
                if curr.next is not None:
                    x = str(curr.next.data)  # Get data of node being deleted
                    curr.next = curr.next.next  # Remove node
            #        print("delete " + x)
                    return x  # Return deleted node data
            cnt += 1
            curr = curr.next
